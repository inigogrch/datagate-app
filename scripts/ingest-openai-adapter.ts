...
async function main() {
  console.log('🚀 Starting OpenAI Official Blog ingestion...')
  console.log('Time:', new Date().toISOString())
  
  const supabase = createAdminClient()
  
  try {
    // Fetch OpenAI source from database
    const { data: source, error: sourceError } = await supabase
      .from('sources')
      .select('id, name')
      .eq('name', 'OpenAI Official Blog')
      .single()
    
    if (sourceError || !source) {
      console.error('❌ OpenAI Official Blog source not found in database')
      console.log('Make sure the URL has been updated in the database')
      process.exit(1)
    }
    ...
    for (const item of items) {
      ...
      // Insert story with both source columns
    ...
    console.log('\n🎉 OpenAI ingestion completed successfully!')
...
export { main as ingestOpenAIFeed } 